...
UmlShapeCoordinates = Dict[str, UmlShapeCoordinate]

UmlMoveNodes = UmlClass | UmlNote
...
class OrthogonalAdapter:
    ...
    def __init__(self, umlShapes: UmlShapes):

        self.logger: Logger      = getLogger(__name__)
        gmlExporter: GMLExporter = GMLExporter()            # TODO: Fix this Yeah I know bad to share this way

        gmlExporter.translate(umlShapes=umlShapes)

        if ExtensionsPreferences().debugTempFileLocation is True:
    ...
    def doLayout(self, layoutAreaSize: LayoutAreaDimensions):
        ...
        self._umlShapeCoordinates = self._toUmlPositionDictionary(nxGraph=self._nxGraph)
    ...
    def _toUmlPositionDictionary(self, nxGraph: Graph) -> UmlShapeCoordinates:

        retCoordinates: UmlShapeCoordinates = dict()
        for node in nxGraph:
            scrCoordinates: ScreenCoordinates  = self._ets.getScreenPosition(node)
            oglCoordinates: UmlShapeCoordinate = UmlShapeCoordinate(scrCoordinates.x, scrCoordinates.y)
